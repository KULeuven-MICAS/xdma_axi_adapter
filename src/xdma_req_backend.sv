...
module xdma_req_backend #(
  ...
  always_comb begin : proc_axi_packer
    ...
    // A ToRemoteData (is_write_data) write holds its AW until its W data is ready
    // (w_valid = grant && write_req_data_valid_i). Ungated, an intermediate node in a
    // multi-hop transfer issues its data-forward AW to the NEXT hop the instant its own grant
    // arrives -- before it has folded any data -- which locks that output port in the D2D
    // write-aware router with no W to release it. Because all off-chip traffic serializes into
    // one soc->D2D stream under AXI W-ordering, that also blocks this node's OUTGOING grant to
    // its previous hop, which then never sends data, so the W never becomes ready: deadlock.
    // Gating on w_valid lets the grant go first. Ordinary writes already have their data, so
    // w_valid is high and the AW is not delayed.
