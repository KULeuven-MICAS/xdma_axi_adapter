...
module xdma_write_demux #(
  ...
  always_comb begin : proc_compose_output
    ...
    // An unmapped address is still handshaken, and dropped. This demux sits directly
    // behind the XDMA's AXI slave port, so holding both VALID and READY low would leave the
    // beat unable to retire: the port stops accepting AW/W, never emits B, and one stray
    // access -- a foreign chip id, a default-routed transaction, a debug write -- blocks the
    // cfg, grant and finish traffic sharing that port until reset. There is no error output
    // to route to, and a lost stray write is recoverable where a wedged port is not.
